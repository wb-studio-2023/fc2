...
<div id="main">
    <section id="regist">
        <h2>{{ $pageName }}</h2>
        <div class="regist">
            <form method="POST" action="{{ route('administrator.actress.edit.confirm') }}" enctype="multipart/form-data">
            @csrf
                @if ($errors->any())
                ...
                <table>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>
                                <input type="text" name="name" value="{{ old('name', $actressData->name) }}">
                            </td>
                        </tr>
                        <tr>
                            <th>
                            ...
                            <td>
                                <input type="text" name="name_kana" value="{{ old('name_kana', $actressData->name_kana) }}">
                            </td>
                        </tr>
                        <tr>
                            <th>アイキャッチ</th>
                            <td class="image_area">
                                ...
                                <img id="thumb">
                                <input type="hidden" id="eyecatch_text" name="eyecatch_text" value="{{ old('eyecatch_text', $actressData->eyecatch) }}">
                            </td>
                            ...
                            <td>
                                <select name="size" class="size">
                                    ...
                                    @foreach (config('const.ACTRESS.SIZE') as $size)
                                        <option value="{{ $size }}"
                                            @if ( $size == old('size', $actressData->size) )
                                                selected 
                                            @endif
                                        >
                                            {{ $size }}
                                            ...
                                            @foreach ($actressTypeList as $actressTypeData)
                                                <label for="actress_type_{{ $actressTypeData->id }}">
                                                    <input type="checkbox" id="actress_type_{{ $actressTypeData->id }}" value="{{ $actressTypeData->id }}" name="actress_type[]"
                                                        @if ( !empty(old('actress_type', $actressData->type)) && in_array($actressTypeData->id, old('actress_type', $actressData->type)) )
                                                            checked 
                                                        @endif
                                                    >
                        ...
                        <tr>
                            ...
                            <td>
                                <textarea name="introduction">{{ old('introduction', $actressData->introduction) }}</textarea>
                            </td>
                        </tr>
                    </tbody>
                </table>
                <input type="hidden" name="id" value="{{ $actressData->id }}">
